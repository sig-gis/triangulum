...
3. Configure the Triangulum features you want to use in a ~config.edn~ file in the root directory of your repository.
   ...
   NOTE: If you would like to use an alternative file name or location
   for your ~config.edn~ file, you should set its path in the
   ~TRIANGULUM_CONFIG~ environment variable before starting your JVM.
...
1. In config.edn:
   ...
   #+begin_src clojure
    :server   {:http-port             8080
               ...
               :private-request-keys  #{:password :passwordConfirmation} ; Redacted from request logs; this is the default
               :private-response-keys #{} ; Redacted from response logs; none by default
   ...
   ~:private-request-keys~ and ~:private-response-keys~ are dropped from
   =wrap-request-logging=/=wrap-response-logging= output before it's written
   to the log. A key matches under either spelling - as a keyword or as its
   string name - since request params are keywordized while JSON response
   bodies are parsed with string keys.
...
3)  ~triangulum.handler/authenticated-routing-handler~ will use any custom
   implementation of these handlers that you specify in your ~config.edn~:
    ...
    - ~:refused-handler~ (args: ~request~), optional, called when client is not
      authenticated and the route does not redirect. Defaults to a bare 403
      ~Forbidden~; register your own to say *why* the request was refused --
      a session that has ended is not the same as a permission the caller
      never had, and a 403 cannot tell them apart.

#+begin_src clojure
  ;; nested config
  {:server {:not-found-handler   product-ns.handlers/not-found-handler
            :redirect-handler    product-ns.handlers/redirect-handler
            :refused-handler     product-ns.handlers/refused-handler
  ...
  {:triangulum.handler/not-found-handler   product-ns.handlers/not-found-handler
   ...
   :triangulum.handler/refused-handler     product-ns.handlers/refused-handler
