...
Then add ~"src/sql"~ directory to the ~:paths~ section in your
~deps.edn~ file.
...
Triangulum to create a ~.service~ file by adding the following to
your ~:aliases~ section in the ~deps.edn~ file:

#+begin_src clojure
{:aliases {:build-db {:main-opts ["-m" "triangulum.systemd"]}}}
#+end_src

And then running:
#+begin_src sh
clojure -M:systemd --enable -p <PATH> -r <REPO> -u <USER> [-o OFFSET]
#+end_src
...
enabled your service. Now your service will be enabled at startup.
...
Add to the ~:aliases~ section in ~deps.edn~.
