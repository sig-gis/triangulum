...
In dev mode, ~js-init~ (file exporting ~pageInit~ for rendering root component) relies on a running Vite local server. Vite serves JavaScript (JSX), enabling quick reloads and efficient builds.In ClojureScript projects it is passed under the key ~:cljs-init~. ~pageInit~ is a function that expects to receive request `params` and `session` maps.
