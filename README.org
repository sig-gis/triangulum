...
a function called ~pageInit~ that expects two arguments: ~session~ and
~params~. You only need to set this key for the development mode to work,
...
In ClojureScript projects, we need to pass the namespaced symbol of the init
function to the ~:cljs-init~ key, which accepts ~session~ and ~params~ as
